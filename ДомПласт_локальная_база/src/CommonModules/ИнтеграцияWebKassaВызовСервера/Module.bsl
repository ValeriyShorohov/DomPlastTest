...
Функция ПечатьФискальногоЧека(СтруктураДанных, ТипЧека, Параметры, ТабДок, ДополнительныеДанные = Неопределено) Экспорт
	...
	Попытка
		...
		ОблШапкаОрг.Параметры.ИИН = "БИН " + ДополнительныеДанные.ИИН;
		ОблШапкаОрг.Параметры.ДокументНДС = "НДС Серия " + ДополнительныеДанные.СерияСвидетельстваПоНДС
			+ " № " + ДополнительныеДанные.НомерСвидетельстваПоНДС;
		ОблШапкаОрг.Параметры.ИНК = "ИНК: " + Формат(СтруктураДанных.ИНК, ФорматБезПробела);
		ОблШапкаОрг.Параметры.РНК = "РНК: " + СтруктураДанных.РегистрационныйНомерККМ;
		ОблШапкаОрг.Параметры.ЗНК = "ЗНК: " + СтруктураДанных.СерийныйНомерККМ;
		ТабДок.Вывести(ОблШапкаОрг);
		
		ОблШапкаЧека = Макет.ПолучитьОбласть("ШапкаЧека | _" + ШиринаЛенты);
		ОблШапкаЧека.Параметры.Кассир = "Кассир: " + ДополнительныеДанные.Кассир;
		ОблШапкаЧека.Параметры.НомерСмены = "Смена №" + Формат(СтруктураДанных.НомерСмены, ФорматБезПробела);
		ОблШапкаЧека.Параметры.НомерЧека = "Чек №" + Формат(СтруктураДанных.НомерВРамкахСмены, ФорматБезПробела);
		
		Если СтруктураДанных.OperationType = 0 Тогда
			ОблШапкаЧека.Параметры.ТипЧека = "ПОКУПКА";
		ИначеЕсли СтруктураДанных.OperationType = 1 Тогда
			ОблШапкаЧека.Параметры.ТипЧека = "ВОЗВРАТ ПОКУПКИ";
		ИначеЕсли СтруктураДанных.OperationType = 2 Тогда
			ОблШапкаЧека.Параметры.ТипЧека = "ПРОДАЖА";
		ИначеЕсли СтруктураДанных.OperationType = 3 Тогда
			ОблШапкаЧека.Параметры.ТипЧека = "ВОЗВРАТ ПРОДАЖИ";
		КонецЕсли;
		...
		Для Каждого Позиция Из СтруктураДанных.Positions Цикл
			...
			Если Позиция.Discount <> 0 Тогда
				ОблТелоЧекаСкидка.Параметры.ЗагСкидка = "Скидка";
			...
			Если Позиция.Markup <> 0 Тогда
				ОблТелоЧекаСкидка.Параметры.ЗагСкидка = "Наценка";
			...
			Если Позиция.TaxType <> 0 И Позиция.Tax <> 0 Тогда
				...
				ОблТелоЧекаСкидка.Параметры.ЗагСкидка = СтрШаблон("НДС %1%%", СтавкаНДС);
				ОблТелоЧекаСкидка.Параметры.Скидка = Формат(Позиция.Tax, ФорматДенежный);
				ТабДок.Вывести(ОблТелоЧекаСкидка);
			Иначе
				ОблТелоЧекаСкидка.Параметры.ЗагСкидка = "НДС 0";
				ОблТелоЧекаСкидка.Параметры.Скидка = Формат(Позиция.Tax, ФорматДенежный);
				ТабДок.Вывести(ОблТелоЧекаСкидка);
			КонецЕсли;
		...
		Для Каждого Платеж Из СтруктураДанных.Payments Цикл
			...
			ИначеЕсли Платеж.Свойство("PaymentTypeName") Тогда
				ОблПодвалЧекаОплата.Параметры.ТипОплаты = Платеж.PaymentTypeName + ":";
			КонецЕсли;
		...
		ОблПодвалОФД.Параметры.ФискальныйЧек =
			"Фискальный чек №" + Формат(СтруктураДанных.ФискальныйНомер, ФорматБезПробела);
		
		ОблПодвалОФД.Параметры.ВремяЧека = "Время: " + Формат(СтруктураДанных.ДатаЧека, ФорматДаты);
		ОблПодвалОФД.Параметры.АдресККМ = "Адрес: " + СтруктураДанных.АдресККМ;
		ОблПодвалОФД.Параметры.КодККМ = "Код ККМ: " + СтруктураДанных.РегистрационныйНомерККМ;
		...
		Если СтруктураДанных.АвтономныйРежимРаботы Тогда
			ОблПодвалОФД.Параметры.АвтономныйРежим = НСтр("ru='Чек сформирован в автономном режиме'");
		КонецЕсли;
...
Функция ПечатьЧекаВнесенияИзъятия(СтруктураДанных, Сумма, ТипЧека, Параметры, ТабДок, ДополнительныеДанные = Неопределено) Экспорт
	...
	Попытка
		...
		ОблШапкаОрг.Параметры.ИИН = "БИН " + ДополнительныеДанные.ИНН;
		ОблШапкаОрг.Параметры.ДокументНДС ="НДС Серия " + ДополнительныеДанные.СерияСвидетельстваПоНДС
			+ " № " + ДополнительныеДанные.НомерСвидетельстваПоНДС;
		ОблШапкаОрг.Параметры.ИНК = "ИНК: " + СтруктураДанных.Data.Cashbox.IdentityNumber;
		ОблШапкаОрг.Параметры.РНК = "РНК: " + СтруктураДанных.Data.Cashbox.RegistrationNumber;
		ОблШапкаОрг.Параметры.ЗНК = "ЗНК: " + СтруктураДанных.Data.Cashbox.UniqueNumber;
		ТабДок.Вывести(ОблШапкаОрг);
		...
		ОблТелоИнкассация.Параметры.Кассир = "Кассир: " + ДополнительныеДанные.Кассир;
...
Функция ПечатьКонтрольнойЛенты(СтруктураДанных, Параметры, ТабДок) Экспорт
	...
	Попытка
		...
		ТабДок.Вывести(ОблНефискальная);
		ОблНефискальная.Параметры.НефискальнаяИнформация = "БИН " + Параметры.ИИН;
		ТабДок.Вывести(ОблНефискальная);
		СведенияНДС = "НДС Серия " + Параметры.СерияНДС + " № " + Параметры.НомерНДС;
		ОблНефискальная.Параметры.НефискальнаяИнформация = СведенияНДС;
		ТабДок.Вывести(ОблНефискальная);
		ОблНефискальная.Параметры.НефискальнаяИнформация = "ЗНК: " + Параметры.ЗНК;
		ТабДок.Вывести(ОблНефискальная);
		ТабДок.Вывести(ОблРазделительВнут);
		ОблНефискальная.Параметры.НефискальнаяИнформация = НСтр("ru='КОНТРОЛЬНАЯ ЛЕНТА'");
		ТабДок.Вывести(ОблНефискальная);
		ОблНефискальная.Параметры.НефискальнаяИнформация = "Смена №" + Формат(Параметры.НомерСмены, ФорматБезПробела);
		ТабДок.Вывести(ОблНефискальная);
		...
		Для Каждого Позиция Из СтруктураДанных.Data Цикл
			...
			ОблСуммаОперации.Параметры.ЗагСкидка = НСтр("ru='Сумма'");
		...
		ОблНефискальная.Параметры.НефискальнаяИнформация = НСтр("ru='*** Конец ленты ***'");
		ТабДок.Вывести(ОблНефискальная);
