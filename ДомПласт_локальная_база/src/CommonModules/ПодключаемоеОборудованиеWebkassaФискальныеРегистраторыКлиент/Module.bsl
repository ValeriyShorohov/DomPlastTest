...
Процедура НачатьФискализациюЧека(ОповещениеПриЗавершении, ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	Для Каждого ПозицияЧека Из ПозицииЧека Цикл
		...
		Если ПозицияЧека.Свойство("ТоварВС") И ПозицияЧека.Свойство("GTIN") И ПозицияЧека.Свойство("ИдентификаторВС") Тогда
			ДанныеДляВС = Новый Структура;
			ДанныеДляВС.Вставить("ТипСклада", ?(ПозицияЧека.ТоварВС, 1, 0));
			ДанныеДляВС.Вставить("GTIN", ?(ПозицияЧека.ТоварВС, ПозицияЧека.GTIN, ""));
			ДанныеДляВС.Вставить("ИдентификаторТовара", ?(ПозицияЧека.ТоварВС, ПозицияЧека.ИдентификаторВС, 0));
		Иначе
			ДанныеДляВС = Неопределено;
		КонецЕсли;
		ДобавитьПозициюВЧек(Наименование, Количество, Цена, Сумма, НомерСекции, СтавкаНДС, ЕдиницаИзмеренияКод, КодМаркировки, ДанныеДляВС, ДанныеДляЗапроса);
...
Функция ДобавитьПозициюВЧек(Наименование, Количество, Цена, Сумма,
							...
							КодМаркировки, ДанныеДляВС, ДанныеДляЗапроса) Экспорт
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(ДанныеДляВС) Тогда
		ПозицияВЧеке.Вставить("GTIN",          ДанныеДляВС.GTIN);
		ПозицияВЧеке.Вставить("ProductId",     ДанныеДляВС.ИдентификаторТовара);
		ПозицияВЧеке.Вставить("WarehouseType", ДанныеДляВС.ТипСклада);
	КонецЕсли;
...
Функция СформироватьТекстФискальногоЧека(СтруктураДанных, Параметры, ДополнительныеДанные = Неопределено) Экспорт
	...
	Попытка
		...
		Если ДополнительныеДанные.Свойство("ЭтоДубликат") Тогда
			Текст = Текст + ПостроитьПолеПереносом(нСтр("ru='ДУБЛИКАТ'", "ru") , ШиринаСтроки, , Истина) + Символы.ПС;
		КонецЕсли;
		Текст = Текст + ПостроитьПолеПереносом(ДополнительныеДанные.Организация, ШиринаСтроки, , Истина) + Символы.ПС;
		Текст = Текст + ВыстроитьПоля("БИН " + ДополнительныеДанные.ИИН, , ШиринаСтроки) + Символы.ПС;
		СведенияНДС = "НДС Серия " + ДополнительныеДанные.СерияСвидетельстваПоНДС
		...
		Текст = Текст + ВыстроитьПоля("ИНК: " + Формат(СтруктураДанных.ИНК, ФорматБезПробела), , ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыстроитьПоля("РНК: " + СтруктураДанных.РегистрационныйНомерККМ, , ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыстроитьПоля("ЗНК: " + СтруктураДанных.СерийныйНомерККМ, , ШиринаСтроки) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		Текст = Текст + ПостроитьПолеПереносом("Кассир: " + ДополнительныеДанные.Кассир, ШиринаСтроки, , Истина) + Символы.ПС;
		Текст = Текст + ПостроитьПоле("Смена №" + Формат(СтруктураДанных.НомерСмены, ФорматБезПробела),ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ПостроитьПоле("Чек №" + Формат(СтруктураДанных.НомерВРамкахСмены, ФорматБезПробела),ШиринаСтроки) + Символы.ПС;
		ТипЧека = "";
		Если СтруктураДанных.OperationType = 0 Тогда
			ТипЧека = "ПОКУПКА";
		ИначеЕсли СтруктураДанных.OperationType = 1 Тогда
			ТипЧека = "ВОЗВРАТ ПОКУПКИ";
		ИначеЕсли СтруктураДанных.OperationType = 2 Тогда
			ТипЧека = "ПРОДАЖА";
		ИначеЕсли СтруктураДанных.OperationType = 3 Тогда
			ТипЧека = "ВОЗВРАТ ПРОДАЖИ";
		КонецЕсли;
		...
		Если СтруктураДанных.Свойство("CustomerXin") И ЗначениеЗаполнено(СтруктураДанных.CustomerXin) Тогда
			ИнформацияИИН = нСтр("ru='ИИН/БИН покупателя:'", "ru") + СтруктураДанных.CustomerXin;
		...
		Для Каждого Позиция Из СтруктураДанных.Positions Цикл
			...
			Если Позиция.Discount <> 0 Тогда
				...
				Текст = Текст + ВыстроитьПоля(НСтр("ru='Скидка'"), Скидка, ШиринаСтроки) + Символы.ПС;
			...
			Если Позиция.Markup <> 0 Тогда
				...
				Текст = Текст + ВыстроитьПоля(НСтр("ru='Наценка'"), Наценка, ШиринаСтроки) + Символы.ПС;
			...
			Если Позиция.TaxType <> 0 И Позиция.Tax <> 0 Тогда
				...
				Текст = Текст + ВыстроитьПоля(СтрШаблон(НСтр("ru='НДС %1%%'"), СтавкаНДС), НДС, ШиринаСтроки) + Символы.ПС;
			Иначе
				НДС = Формат(Позиция.Tax, ФорматДенежный);
				Текст = Текст + ВыстроитьПоля(НСтр("ru='НДС 0'"), НДС, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			
			ИтогоСумма = Формат(СуммаСтроки, ФорматДенежный);
			Текст = Текст + ВыстроитьПоля(НСтр("ru='Стоимость'"), ИтогоСумма, ШиринаСтроки) + Символы.ПС;
		...
		Для Каждого Платеж Из СтруктураДанных.Payments Цикл
			...
			ИначеЕсли Платеж.Свойство("PaymentTypeName") Тогда
				ТипОплаты = Платеж.PaymentTypeName + ":";
			КонецЕсли;
		...
		Если СуммаНаценки <> 0 Тогда
			ТипОплаты = НСтр("ru='Наценка:'");
			Наценка = Формат(СуммаНаценки, ФорматДенежный);
			ТекстВрем = ПостроитьПоле(ТипОплаты,Цел(ШиринаСтроки/2), , Ложь);
			Текст = Текст + ВыстроитьПоля(ТекстВрем, Наценка, ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		
		ТипОплаты = НСтр("ru='Итого:'");
		ИтогоСумма = Формат(СуммаПоЧеку, ФорматДенежный);
		ТекстВрем = ПостроитьПоле(ТипОплаты,Цел(ШиринаСтроки/2), , Ложь);
		Текст = Текст + ВыстроитьПоля(ТекстВрем, ИтогоСумма, ШиринаСтроки) + Символы.ПС;
		
		ТипОплаты = НСтр("ru='НДС:'");
		НДС = Формат(СуммаНДС, ФорматДенежный);
		ТекстВрем = ПостроитьПоле(ТипОплаты,Цел(ШиринаСтроки/2), , Ложь);
		Текст = Текст + ВыстроитьПоля(ТекстВрем, НДС, ШиринаСтроки) + Символы.ПС;
		
		ФискальныйЧек = НСтр("ru='Фискальный чек №'") + Формат(СтруктураДанных.ФискальныйНомер, ФорматБезПробела);
		ВремяЧека = НСтр("ru='Время: '") + Формат(СтруктураДанных.ДатаЧека, ФорматДаты);
		АдресККМ = НСтр("ru='Адрес: '") + Формат(СтруктураДанных.АдресККМ, ФорматДаты);
		КодККМ = НСтр("ru='Код ККМ: '") + СтруктураДанных.РегистрационныйНомерККМ;
		...
		Если ЗначениеЗаполнено(ОписаниеОФД) Тогда
			Текст = Текст + ПостроитьПолеПереносом(СтрШаблон(НСтр("ru='Оператор фискальных данных: %1'"), ОписаниеОФД.Name), ШиринаСтроки) + Символы.ПС;
			Текст = Текст + ПостроитьПолеПереносом(НСтр("ru='Для проверки чека зайдите на сайт:'"), ШиринаСтроки) + Символы.ПС;
		...
		Если СтруктураДанных.АвтономныйРежимРаботы Тогда
			Текст = Текст + ПостроитьПолеПереносом(НСтр("ru='Чек сформирован в автономном режиме'"), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Текст = Текст + РазделительВнут;
		Текст = Текст + ПостроитьПолеПереносом(НСтр("ru='ФИСКАЛЬНЫЙ ЧЕК ФП'"), ШиринаСтроки,,Истина) + Символы.ПС;
...
Функция СформироватьТекстВнесенияИзъятия(СтруктураДанных, Сумма, ТипЧека, Параметры, ДополнительныеДанные = Неопределено) Экспорт
	...
	Попытка
		...
		Текст = Текст + ВыстроитьПоля("БИН " + ДополнительныеДанные.ИНН, , ШиринаСтроки) + Символы.ПС;
		СведенияНДС = "НДС Серия " + ДополнительныеДанные.СерияСвидетельстваПоНДС
			+ " № " + ДополнительныеДанные.НомерСвидетельстваПоНДС;
		Текст = Текст + ПостроитьПолеПереносом(СведенияНДС, ШиринаСтроки, , Истина) + Символы.ПС;
		Текст = Текст + ВыстроитьПоля("ИНК: " + Формат(СтруктураДанных.Data.Cashbox.IdentityNumber, ФорматБезПробела), , ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыстроитьПоля("РНК: " + Формат(СтруктураДанных.Data.Cashbox.RegistrationNumber, ФорматБезПробела), , ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыстроитьПоля("ЗНК: " + СтруктураДанных.Data.Cashbox.UniqueNumber, , ШиринаСтроки) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		
		ТипИнкассации = "";
		Если ТипЧека = 0 Тогда
			ТипИнкассации = "ВНЕСЕНИЕ";
		ИначеЕсли ТипЧека = 1 Тогда
			ТипИнкассации = "ВЫПЛАТА";
		КонецЕсли;
		...
		Текст = Текст + ВыстроитьПоля("Кассир: " + ДополнительныеДанные.Кассир, , ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыравнитьПоле(НСтр("ru='Сумма'"),ШиринаСтроки,"Право") + Символы.ПС;
...
Функция СформироватьТекстКонтрольнойЛенты(СтруктураДанных, Параметры) Экспорт
	...
	Попытка
		...
		Текст = Текст + ВыстроитьПоля("БИН " + Параметры.ИИН, , ШиринаСтроки) + Символы.ПС;
		СведенияНДС = "НДС Серия " + Параметры.СерияНДС
			+ " № " + Параметры.НомерНДС;
		Текст = Текст + ПостроитьПолеПереносом(СведенияНДС, ШиринаСтроки, , Истина) + Символы.ПС;
		Текст = Текст + ВыстроитьПоля("ЗНК: " + Параметры.ЗНК, , ШиринаСтроки) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		Текст = Текст + ВыстроитьПоля(НСтр("ru='КОНТРОЛЬНАЯ ЛЕНТА'"), , ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыстроитьПоля("Смена №" + Формат(Параметры.НомерСмены, ФорматБезПробела),,ШиринаСтроки) + Символы.ПС;
		...
		Для Каждого Позиция Из СтруктураДанных.Data Цикл
			...
			Иначе
				Текст = Текст + РазделительВнут;
			КонецЕсли;
			Текст = Текст + ПостроитьПоле(Позиция.OperationTypeText, ШиринаСтроки) + Символы.ПС;
			ВремяОперации = НСтр("ru='Время: '") + Формат(Позиция.Date, ФорматДаты);
			Текст = Текст + ПостроитьПоле(ВремяОперации, ШиринаСтроки) + Символы.ПС;
			Если Позиция.Number <> Неопределено Тогда
				ФискальныйЧек = НСтр("ru='Фискальный чек №'") + Формат(Позиция.Number, ФорматБезПробела);
				Текст = Текст + ПостроитьПоле(ФискальныйЧек,ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Сумма = Формат(Позиция.Sum, ФорматДенежный);
			Текст = Текст + ВыстроитьПоля(НСтр("ru='Сумма'"), Сумма, ШиринаСтроки) + Символы.ПС;
		КонецЦикла;
		
		Текст = Текст + ПостроитьПолеПереносом(НСтр("ru='*** Конец ленты ***'"), ШиринаСтроки, , Истина) + Символы.ПС;
