...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ЗаполнитьПоБИН.Видимость									   = ЭтоЮрЛицо ИЛИ Объект.ИндивидуальныйПредпринимательАдвокатЧастныйНотариус; 
...
// Процедура - Заполнить реквизиты по данным КГДНа клиенте
//
&НаКлиенте
...
Процедура ЗаписьЭлементаПолеБИНПослеВопроса(Результат,ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Объект.Наименование 			    = СокращенноеНаименованиеКонтрагента(ДополнительныеПараметры.ОтветСервиса.commonOrgData.name);
		Объект.ИдентификационныйКодЛичности = ДополнительныеПараметры.ОтветСервиса.commonOrgData.bin;
		
		Если ДополнительныеПараметры.ОтветСервиса.commonOrgData.ip Тогда
		    Объект.ЮрФизЛицо = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ФизЛицо");
			Объект.ИндивидуальныйПредпринимательАдвокатЧастныйНотариус = Истина;
			Объект.НаименованиеПолное = ДополнительныеПараметры.ОтветСервиса.commonOrgData.fio;
			УправлениеФормой(ЭтаФорма);
		Иначе	
			...
			Объект.НаименованиеПолное = ?(ДополнительныеПараметры.ОтветСервиса.commonOrgData.fullName <> Неопределено
										, ДополнительныеПараметры.ОтветСервиса.commonOrgData.fullName
										, ДополнительныеПараметры.ОтветСервиса.commonOrgData.name);
			УправлениеФормой(ЭтаФорма);
		КонецЕсли; 

		Записать();
		
		ЗаполнитьКИ(ДополнительныеПараметры.ОтветСервиса);
		//Заполнить ФИО
		МассивФИО = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ДополнительныеПараметры.ОтветСервиса.commonOrgData.fio," ");
		
		ДанныеЗаполненияКЛ = Новый Структура();
		ДанныеЗаполненияКЛ.Вставить("Владелец", Объект.Ссылка);
		
		Если МассивФИО.Количество() = 1 Тогда 
			ДанныеЗаполненияКЛ.Вставить("Фамилия"	,МассивФИО[0]);
		ИначеЕсли МассивФИО.Количество() = 2 Тогда
			ДанныеЗаполненияКЛ.Вставить("Фамилия"	,МассивФИО[0]);
			ДанныеЗаполненияКЛ.Вставить("Имя"		,МассивФИО[1]);
		ИначеЕсли МассивФИО.Количество() = 3 Тогда
			ДанныеЗаполненияКЛ.Вставить("Фамилия"	,МассивФИО[0]);
			ДанныеЗаполненияКЛ.Вставить("Имя"		,МассивФИО[1]);
			ДанныеЗаполненияКЛ.Вставить("Отчество"	,МассивФИО[2]);
		КонецЕсли;   
		
		ЗаполнитьДанныеКонтактногоЛица(ДанныеЗаполненияКЛ);
		
		//Заполним/создадим ФизЛицо
		Если Объект.ЮрФизЛицо = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ФизЛицо") Тогда 		
			ДанныеЗаполненияКЛ.Вставить("ФИОФизЛица", Объект.НаименованиеПолное);
			ДанныеЗаполненияКЛ.Вставить("ИИН", Объект.ИдентификационныйКодЛичности); 
			ЗаполнитьДанныеФизЛица(ДанныеЗаполненияКЛ);
		КонецЕсли;     
...
&НаКлиенте
Функция СокращенноеНаименованиеКонтрагента(ИсходноеНаименование)

	Если СтрДлина(ИсходноеНаименование) < 50 Тогда
		Возврат ИсходноеНаименование;
	Иначе
		ПерваяКавычка = СтрНайти(ИсходноеНаименование,"""");
		ПоследняяКавычка = СтрНайти(ИсходноеНаименование,"""",НаправлениеПоиска.СКонца);
		УкороченнаяСтрока = Сред(ИсходноеНаименование,ПерваяКавычка+1,(ПоследняяКавычка-ПерваяКавычка));//Лев(ИсходноеНаименование, ПоследняяКавычка-1);
		Возврат СтрЗаменить(УкороченнаяСтрока,"""","");
	КонецЕсли;

КонецФункции // СокращенноеНаименованиеКонтрагента()

&НаСервере
Процедура ЗаполнитьКИ(ДанныеДляЗаполнения)
	
	ПараметрыЗаполнения = Новый Массив();
	
	ДанныеЮрАдреса = Новый Структура("Тип, Вид, Представление");
		
	ВидЮрАдреса = Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента;
	ДанныеЮрАдреса.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес;
	ДанныеЮрАдреса.Вид = ВидЮрАдреса;
	ДанныеЮрАдреса.Представление = ДанныеДляЗаполнения.commonOrgData.katoAddress;
	
	ПараметрыЗаполнения.Добавить(ДанныеЮрАдреса);
	
	ДанныеТелефон = Новый Структура("Тип, Вид, Представление");
		
	ВидТелефон = Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента;
	ДанныеТелефон.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон;
	ДанныеТелефон.Вид = ВидТелефон;
	ДанныеТелефон.Представление = ДанныеДляЗаполнения.goszakup.phone;
	
	ПараметрыЗаполнения.Добавить(ДанныеТелефон);

	Если ЗначениеЗаполнено(ДанныеДляЗаполнения.goszakup.email) Тогда
		ДанныеЭлАдреса = Новый Структура("Тип, Вид, Представление");

		ВидЭлАдреса = ВидКИНаСервере(Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты, "Электронный адрес контрагента");
		ДанныеЭлАдреса.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты;
		ДанныеЭлАдреса.Вид = ВидЭлАдреса;
		ДанныеЭлАдреса.Представление = ДанныеДляЗаполнения.goszakup.email; 
		
		ПараметрыЗаполнения.Добавить(ДанныеЭлАдреса);
	КонецЕсли;
	 	
	РегистрыСведений.КонтактнаяИнформация.ЗаполнитьРегистрАдресныхДанных(Объект.Ссылка, ПараметрыЗаполнения);
	
КонецПроцедуры 

&НаСервере
Процедура ЗаполнитьДанныеКонтактногоЛица(ДанныеЗаполнения)
	
	НовоеКонтактноеЛицо = Справочники.КонтактныеЛица.ДобавленноеКонтактноеЛицо(ДанныеЗаполнения);
	Если НовоеКонтактноеЛицо <> Неопределено И НЕ ЗначениеЗаполнено(Объект.ОсновноеКонтактноеЛицо) Тогда
		Объект.ОсновноеКонтактноеЛицо = НовоеКонтактноеЛицо;
		Записать();
	КонецЕсли; 
...
&НаСервере
Процедура ЗаполнитьДанныеФизЛица(ДанныеЗаполнения)
	
	НовоеФизЛицо = Справочники.ФизическиеЛица.СоздатьНайтиФизическоеЛицо(ДанныеЗаполнения);
	Если НовоеФизЛицо <> Неопределено И НЕ ЗначениеЗаполнено(Объект.ФизЛицо) Тогда
		Объект.ФизЛицо = НовоеФизЛицо;
		Записать();
	КонецЕсли; 
...
&НаСервереБезКонтекста
Функция ВидКИНаСервере(ТипКИ, НаименованиеКИ)
	...
	Запрос.Текст = 
		...
		|			ТОГДА ВЫБОР
		|					КОГДА ВидыКонтактнойИнформации.Наименование = ""Электронный адрес контрагента""
		|						ТОГДА ВидыКонтактнойИнформации.Ссылка
		|					ИНАЧЕ NULL
		|				КОНЕЦ
		|		ИНАЧЕ ВидыКонтактнойИнформации.Ссылка
		|	КОНЕЦ КАК Ссылка
		|ИЗ
	...
	Запрос.УстановитьПараметр("ВидОбъекта", Перечисления.ВидыОбъектовКонтактнойИнформации.Контрагенты);
	...
	НовыйОбъектКИ.Наименование 					 = НаименованиеКИ;
	НовыйОбъектКИ.Тип	                         = ТипКИ;
	НовыйОбъектКИ.ВидОбъектаКонтактнойИнформации = Перечисления.ВидыОбъектовКонтактнойИнформации.Контрагенты;
	НовыйОбъектКИ.Записать();
	
	Возврат НовыйОбъектКИ.Ссылка;
		
	
КонецФункции // ВидКИНаСервере()
