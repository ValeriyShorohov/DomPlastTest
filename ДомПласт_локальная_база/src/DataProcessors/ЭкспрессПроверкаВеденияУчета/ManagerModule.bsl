...
Процедура ПроверитьПравилоСвоевременнаяВыпискаЭСФ(СтрокаПравила, ПараметрыОтчета)
	...
	|			ТОГДА ВТ_ДокументыОтгрузки.Дата
	...
	|		ПО ВТ_ДокументыОтгрузки.Ссылка = ВТ_СФ.ДокументОснование
	|ГДЕ
	|	ВЫБОР
...
Процедура ЗаполнитьДеревоПроверокИзМакетаMXL(ДеревоПроверок, ДатаНач = Неопределено) Экспорт
	...
	Для НомСтроки = НомПервойСтрокиПравил По НомПоследнейСтрокиПравил Цикл
		...
		Если ЕстьИмяУСтроки Тогда
			...
			Если (ИмяОбластиСтроки = "МинусыПоТоварамОрганизацийБУ" ИЛИ ИмяОбластиСтроки = "ПроверкаВыпискиЭСФИИП")
					И Константы.ДатаПереходаНаУчетПоТоварамОрганизацийНаВиртуальныхСкладах.Получить() <= ДатаНач Тогда
				ЗначениеФО = Ложь;
			КонецЕсли;
