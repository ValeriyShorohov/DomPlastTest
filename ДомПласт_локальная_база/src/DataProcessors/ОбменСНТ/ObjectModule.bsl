...
Процедура ДокументСНТ_ОбработкаПроведения(ОбъектСНТ, Отказ, РежимПроведения) Экспорт
	...
	Если ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Исходящий 
		И (ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.ПеремещениеМеждуФилиалами И ОбъектСНТ.ВидПеремещения <> Перечисления.ВидыПеремещенияСНТ.ВПределахОдногоЛицаЕАЭС)
	...
	Если (((ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Входящий И НЕ СНТСервер.ПроверитьСтатусНеТребуетПодтверждения(ОбъектСНТ) И НЕ ОбъектСНТ.ПредоставленВБумажномВиде)
		ИЛИ (ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Исходящий И ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз И НЕ ОбъектСНТ.ПоставщикРозничнаяРеализация)))
...
Процедура ЗаполнитьСНТ_Версия1(Знач SntInfo, Знач СтруктурнаяЕдиница, Знач НаправлениеСНТ, СНТ) Экспорт //Получение СНТ
	...
	Если ogdMarksInfo <> Неопределено Тогда
		...
		СНТ.ДатаВремяПересеченияГосударственнойГраницы = СНТСервер.ДатаИБ(СНТСервер.ПолучитьСвойство(ogdMarks, "borderCrossingDateWithTime", "01.01.0001")); // Дата и время пересечения государственной границы/территории СЭЗ (N 89)
			...
			Если КоллекцияСсылокСНТ.Количество() <> 0 Тогда
				...
				Для Каждого СНТ Из КоллекцияСсылокСНТ Цикл
					Если СНТ.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСНТ.Перемещение") ИЛИ
						СНТ.ТипСНТ = ПредопределенноеЗначение("Перечисление.ТипыСНТ.ВозвратТоваров") ИЛИ Действие = "АннуллироватьПодтверждение" Тогда
						Продолжить;
					КонецЕсли;
...
Функция РезультатЗапросаПоРеквизитамСНТ(ПараметрыЗаполнения) Экспорт
	...
	|		КОГДА СНТ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСНТ.ПеремещениеМеждуФилиалами)
	|				И СНТ.ВидПеремещения = ЗНАЧЕНИЕ(Перечисление.ВидыПеремещенияСНТ.ВПределахОдногоЛицаЕАЭС)
	|			ТОГДА вт_ДанныеПоСобственномуКонтрагенту.ГрузополучательИдентификатор
	|		ИНАЧЕ СНТ.ГрузополучательИдентификатор
	...
	|		КОГДА СНТ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСНТ.ПеремещениеМеждуФилиалами)
	|				И СНТ.ВидПеремещения = ЗНАЧЕНИЕ(Перечисление.ВидыПеремещенияСНТ.ВПределахОдногоЛицаЕАЭС)
	|			ТОГДА вт_ДанныеПоСобственномуКонтрагенту.ПолучательИдентификатор
	|		ИНАЧЕ СНТ.ПолучательИдентификатор
...
Функция ПодготовитьПараметрыПроведенияСНТ(ДокументСсылка, Отказ) Экспорт
	...
	Если (Реквизиты.Направление = Перечисления.НаправленияСНТ.Входящий 
		...
		ИЛИ (Реквизиты.Направление = Перечисления.НаправленияСНТ.Исходящий 
			И (Реквизиты.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз ИЛИ (Реквизиты.ТипСНТ = Перечисления.ТипыСНТ.ВозвратТоваров И Реквизиты.ПоставщикМалаяТорговаяТочка И НЕ ВозвратМТТАкциз))) Тогда
	...
	Для Каждого СтрокаТовар Из ОбъектСНТ[ИмяТабличнойЧасти] Цикл
		...
		Если ТипЗнч(ОбъектСНТ) = Тип("ДокументОбъект.СНТ") Тогда
			...
			Иначе
				...
				Если НЕ (ОбъектСНТ.ВидВвоза = Перечисления.ВидыВвозаСНТ.Импорт И СтрокаТовар.ДополнительныйИдентификатор = 1) Тогда
			...
			Если ИмяТабличнойЧасти <> "Товары" И (ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Входящий ИЛИ (ОбъектСНТ.ВидВвоза = Перечисления.ВидыВвозаСНТ.Импорт И СтрокаТовар.ДополнительныйИдентификатор <> 1)
				ИЛИ ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.ПеремещениеМеждуФилиалами) Тогда 
			...
			Если ОбъектСНТ.Направление = Перечисления.НаправленияСНТ.Исходящий И ОбъектСНТ.ВидОперации = Перечисления.ВидыОперацийСНТ.Ввоз Тогда
				//если возврат от нерезидента, данные берем из документа, которые зпаполнились из первоначального источника
