...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзмененПериод" Тогда
		ПроверитьНаличиеСтавокОСМС(Объект.ПериодРегистрации, Объект.Организация, Объект.ВидОперации, ЕстьСтавкиОСМСЮрЛица, ЕстьСтавкаВОСМСФизЛица, ЕстьСтавкаООСМСФизЛица);
...
Процедура СведенияОДоходахВидРасчетаПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура("ФизЛицо, ДоговорКонтрагента, МесяцНалоговогоПериода, СпособНалогообложенияИПН, СпособНалогообложенияОПВ, 
											|СпособНалогообложенияСН, СпособНалогообложенияСО, СпособНалогообложенияОППВ, СпособНалогообложенияВОСМС, СпособНалогообложенияООСМС, ДатаДохода, УчитыватьИПН, 
											|УчитыватьОПВ, УчитыватьСН, УчитыватьСО, УчитыватьОППВ, УчитыватьВОСМС, УчитыватьООСМС, ВидРасчета, СуммаДохода, СуммаВычета, СпособОтраженияВБухучете");
...
Процедура СведенияОДоходахВидРасчетаПриИзмененииНаСервере (ДанныеСтрокиТаблицы, ПараметрыОбъекта)
	...
	СведенияОНалогообложенииНачисления = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеСтрокиТаблицы.ВидРасчета, "ОблагаетсяИПН, ОблагаетсяОПВ, 
																						|ОблагаетсяСН, ОблагаетсяСО, ОблагаетсяОППВ, ОблагаетсяВОСМС, ОблагаетсяООСМС");
...
Процедура ПроверитьНаличиеСтавокОСМС(ДатаСреза, Организация, ВидОперации, ЕстьСтавкиОСМСЮрЛица, ЕстьСтавкаВОСМСФизЛица, ЕстьСтавкаООСМСФизЛица)
	
	Если ДатаСреза >= Дата('20200101') Тогда
		Если ВидОперации =  Перечисления.ВидыОперацийРегистрацияПрочихДоходов.ДоходыВНатуральнойФорме Тогда            
			ЕстьСтавкаВОСМСФизЛица     = Истина;
			ЕстьСтавкаООСМСФизЛица    = Истина;
