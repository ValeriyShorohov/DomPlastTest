...
Процедура ТиповойСчетКтПриИзменении(Элемент)
	...
	ПроцедурыБухгалтерскогоУчетаКлиентСервер.ПриИзмененииСчета(СтрокаТаблицы.СчетДт, СтрокаТаблицы, ПоляОбъекта, Ложь);
...
Процедура НалоговыйСчетКтПриИзменении(Элемент)
	...
	ПроцедурыБухгалтерскогоУчетаКлиентСервер.ПриИзмененииСчета(СтрокаТаблицы.СчетКт, СтрокаТаблицы, ПоляОбъекта, Ложь);
...
Процедура УстановитьДоступностьСубконтоВСтрокеТабличнойЧасти(Форма, СтрокаТабличнойЧасти, ИмяТабличнойЧасти)
	...
	ПроцедурыБухгалтерскогоУчетаКлиентСервер.УстановитьДоступностьИЗаголовкиСубконто(СтрокаТабличнойЧасти[ИмяСчета], СтрокаТабличнойЧасти, ПоляОбъекта, Ложь);
...
&НаСервере
Процедура БУЗаполнитьНУНаСервере()
	...
	Для Каждого Выборка Из ТиповойНаборЗаписей Цикл
		...
		Проводка = НалоговыйНаборЗаписей.Добавить();
		...
		Если ЗначениеЗаполнено(СчетДт) Тогда
			...
			Для Индекс = 1 По СчетДт.ВидыСубконто.Количество() Цикл
				Если Проводка["СубконтоДт" + Индекс] = Неопределено Тогда
					Проводка["СубконтоДт" + Индекс] = Новый(СчетДт.ВидыСубконто[Индекс-1].ВидСубконто.ТипЗначения.Типы()[0]);
				КонецЕсли;
		...
		Если ЗначениеЗаполнено(СчетКт) Тогда
			...
			Для Индекс = 1 По СчетКт.ВидыСубконто.Количество() Цикл
				Если Проводка["СубконтоКт" + Индекс] = Неопределено Тогда
					Проводка["СубконтоКт" + Индекс] = Новый(СчетКт.ВидыСубконто[Индекс-1].ВидСубконто.ТипЗначения.Типы()[0]);
				КонецЕсли;
