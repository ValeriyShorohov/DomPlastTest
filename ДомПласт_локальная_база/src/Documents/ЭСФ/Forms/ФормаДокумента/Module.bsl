...
&НаСервере
Процедура ПолучательРозничнаяРеализацияПриИзмененииНаСервере()
	
	СтруктураНомерИДатаПоставки = ЭСФСерверПереопределяемый.ПолучитьНомерДатуДокументаПоставки(ПолучитьЗначениеРеквизита(Объект.СчетФактура,"ДокументОснование"));
...
Процедура ПолучательРозничнаяРеализацияПриИзменении(Элемент)
	...
	Если СтрокаПолучатели.РозничнаяРеализация И (Объект.ДокументПодтверждающийПоставкуНомер = "NON SNT") И НЕ Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияЭСФ.Входящий") Тогда
...
Процедура ПерезаполнитьНомерДокументаПоставкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ПолучательРозничнаяРеализацияПриИзмененииНаСервере();
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	//++ НЕ ЭСФ
	Если Не (СтрНайти(Объект.ДокументПодтверждающийПоставкуНомер, "KZ-SNT-") > 0 ИЛИ СтрНайти(Объект.ДокументПодтверждающийПоставкуАВРНомер, "KZ-SNT-") > 0) Тогда
