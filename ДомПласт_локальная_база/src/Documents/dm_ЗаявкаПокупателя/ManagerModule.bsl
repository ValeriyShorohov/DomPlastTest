...
Функция ДоступныеВидыОпераций() Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ПоддержкаРаботыСоСтруктурнымиПодразделениями") Тогда 
		ИсключаемыеОперации.Добавить(Перечисления.ВидыОперацийРеализацияТоваров.ПередачаСтруктурномуПодразделению);
	КонецЕсли;
	
	Для Каждого ВидОперации Из Перечисления.ВидыОперацийРеализацияТоваров Цикл
...
Процедура ЗаполнитьДокументПоСчетФактураВыданный(Объект, Основание) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивНоменклатуры, ОбщегоНазначения.ВыгрузитьКолонку(Объект.Услуги, "Номенклатура", Истина), Истина);
	
	//СоответствиеСчетовУчета = ПроцедурыБухгалтерскогоУчета.ПолучитьСчетаУчетаСпискаНоменклатуры(
	//		Объект.Организация, МассивНоменклатуры, Объект.Дата);
	//
	//Для Каждого СтрокаТабличнойЧасти Из Объект.Товары Цикл
	//	ЗаполнитьРеквизитыНалоговогоУчета(Объект, СтрокаТабличнойЧасти, "Товары");
	//	ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(Объект, СтрокаТабличнойЧасти, "Товары", СоответствиеСчетовУчета.Получить(СтрокаТабличнойЧасти.Номенклатура));
	//КонецЦикла;
	//
	...
	//	ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(Объект, СтрокаТабличнойЧасти, "Услуги", СоответствиеСчетовУчета.Получить(СтрокаТабличнойЧасти.Номенклатура));
	//КонецЦикла;
	//
КонецПроцедуры
...
Процедура ЗаполнитьПоПоступлению(Объект, ИмяТабЧасти, Режим, Поступление) Экспорт 
	...
	//ЗаполнитьЗначенияСвойств(ДанныеОбъекта, Объект);
	//
	//СоответствиеСведенийОНоменклатуре = ПроцедурыБухгалтерскогоУчета.ПолучитьСчетаУчетаСпискаНоменклатуры(
	...
	//	КонецЕсли;
	//	
	//	ЗаполнитьРеквизитыНалоговогоУчета(ДанныеОбъекта, СтрокаТабличнойЧасти, ИмяТабЧасти);
...
Процедура СкопироватьТовары(Объект, Основание)
	...
	ЗаполнитьСчетаУчетаВТабличнойЧасти(Объект, "Товары");
	...
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		...
		ЗаполнитьРеквизитыНалоговогоУчета(Объект, СтрокаТЧ, "Товары");
...
Процедура СкопироватьУслуги(Объект, Основание)
	...
	ЗаполнитьСчетаУчетаВТабличнойЧасти(Объект, "Услуги");
	...
	Для Каждого СтрокаТЧ Из Объект.Услуги Цикл
		ЗаполнитьРеквизитыНалоговогоУчета(Объект, СтрокаТЧ, "Услуги");
...
Функция ТекстЗапросаРеализацияТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	NULL КАК СубконтоЗатратБУ1,
	|	NULL КАК СубконтоЗатратБУ2,
	|	NULL КАК СубконтоЗатратБУ3,
	|	NULL КАК СчетЗатратНУ,
	|	NULL КАК СубконтоЗатратНУ1,
	|	NULL КАК СубконтоЗатратНУ2,
	|	NULL КАК СубконтоЗатратНУ3,
	|	NULL КАК Партия,
	|	NULL КАК ЭтоУслуга
...
Функция ПечатьАктОбОказанииУслуг(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	//|			ТОГДА ВЫРАЗИТЬ(РеализацияТоваровУслуг.Содержание КАК СТРОКА(250))
...
Функция ПечатьР1(МассивОбъектов, ОбъектыПечати, КолонкиНДС = Ложь) Экспорт
	...
	//	Подвал.Параметры.ДатаПринятия            = Формат(ДанныеДокументов.Дата, "ДЛФ=Д");
	//		
...
Функция ПолучитьДанныеДляПечатиР1(МассивОбъектов)
	...
	//|			ТОГДА ВЫРАЗИТЬ(РеализацияТоваровУслугУслуги.Содержание КАК СТРОКА(250))
