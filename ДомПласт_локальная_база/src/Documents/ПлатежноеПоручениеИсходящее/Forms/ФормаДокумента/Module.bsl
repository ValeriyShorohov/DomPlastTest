...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеПенсионныхВзносов Тогда
		...
		Для Каждого СтрокаТаблицы Из Объект.ПеречислениеПенсионныхВзносов Цикл
			СтрокаТаблицы.ВидВзноса = ?(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Документ, "ВидОперации") = 
					Перечисления.ВидыОперацийОПВПеречислениеВФонды.ПеречислениеОбязательныхПрофессиональныхПенсионныхВзносов, НСтр("ru = 'ОППВ'"), НСтр("ru = 'ОПВ'"));
		КонецЦикла;
				...
				Если ЗначениеЗаполнено(ДокументДляПеречисления) Тогда
					...
					Если ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениАкт
					 ИЛИ ДокументДляПеречисления.ВидПлатежа = Перечисления.ВидыПлатежейВБюджетИФонды.ПениСам Тогда
						...
						Объект.КодНазначенияПлатежа = ?(ВидОПВ = НСтр("ru = 'ОПВ'"), "019", "009");
...
Функция ПолучитьСтруктуруПриИзмененииДокументаППВНаСервере(Документ)
	...
	СтруктураВозврата.ВидВзноса = ?(ПолучитьЗначениеРеквизита(Документ, "ВидОперации") = 
				ПредопределенноеЗначение("Перечисление.ВидыОперацийОПВПеречислениеВФонды.ПеречислениеОбязательныхПрофессиональныхПенсионныхВзносов"), НСтр("ru = 'ОППВ'"), НСтр("ru = 'ОПВ'"));
