...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Если Реквизиты.ЕстьВозвратПенсионныхВзносов
		...
		ИЛИ Реквизиты.ЕстьВозвратЗаработнойПлаты Тогда
...
Функция ТекстЗапросаВозвратОПВ(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Иначе
		...
		|	ВЫБОР
		|		КОГДА ОПВВозвратВзносовПенсионныеВзносы.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОПВВозвратВзносов.ВозвратОбязательныхПенсионныхВзносов)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ВидВзносаОПВ,
		|	Реквизиты.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств
		|ИЗ
