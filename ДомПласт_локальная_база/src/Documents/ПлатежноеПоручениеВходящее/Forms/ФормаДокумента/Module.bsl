...
Процедура ВозвратПенсионныхВзносовДокументПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтрокаВозврата.Документ) Тогда
		...
		СтрокаВозврата.ВидВзноса = ?(ПолучитьЗначениеРеквизита(СтрокаВозврата.Документ, "ВидОперации") = 
					ПредопределенноеЗначение("Перечисление.ВидыОперацийОПВВозвратВзносов.ВозвратОбязательныхПрофессиональныхПенсионныхВзносов"), НСтр("ru = 'ОППВ'"), НСтр("ru = 'ОПВ'"));
	КонецЕсли;  	
	...
	Если  Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратПенсионныхВзносов Тогда
		...
		Для Каждого СтрокаТаблицы Из Объект.ВозвратПенсионныхВзносов Цикл
			СтрокаТаблицы.ВидВзноса = ?(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Документ, "ВидОперации") = 
					Перечисления.ВидыОперацийОПВВозвратВзносов.ВозвратОбязательныхПрофессиональныхПенсионныхВзносов, НСтр("ru = 'ОППВ'"), НСтр("ru = 'ОПВ'"));
		КонецЦикла;
